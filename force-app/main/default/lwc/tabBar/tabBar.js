...
export default class cTabBar extends LightningElement {
    ...
    set endIconName(value) {
        console.log("end icon name1:" + value);
    ...
    handleTabsetEndIconClick(event)
    {
        console.log("tabbarendiconclick");
        event.preventDefault();
        this.dispatchEvent(
            new CustomEvent('tabbarendiconclick', {})
        );
    }
    handleTabEndIconClick(event)
    {
        console.log("tabendicon click");
        event.preventDefault();

        const tabValue = event.target.getAttribute('data-tab-value');
        const tab = this._findTabByValue(tabValue);
        this.dispatchEvent(
            new CustomEvent('endiconclick', {
                detail: {
                    value: tab.value,
                    label: tab.label
                }
